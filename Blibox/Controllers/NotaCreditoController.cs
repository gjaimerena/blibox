...
namespace Blibox.Controllers
{
    ...
    public class NotaCreditoController : Controller
    {
        ...
        public ActionResult Index(string sortOrder, string currentFilter, string q, int page = 1, int pageSize = 10)
        {
            ...
            var query = db.Encabezado_Factura.Include(e => e.Cliente).Include(e => e.Condicion_venta).Include(e => e.Detalle_factura).Include(e => e.Vendedor).Where(m => m.Tipo == "3").OrderBy(m => m.Nro_factura);
        ...
        public ActionResult Create(FormCollection form)
        {
            ...
            int CbteTipoNC = Convert.ToInt32(ConfigurationManager.AppSettings["CbteTipoNC"]);
            int nroRegistros = Convert.ToInt32(ConfigurationManager.AppSettings["nroRegistros"]);
            FECAERespuesta resp = FE.AutorizacionFactura(nroRegistros, ptoVenta, CbteTipoNC, detalles);
            ...
            else
            {
                ...
                {
                    TempData["Noti"] = Notification.Show("Error: no se puede generar en este momento, intente más tarde." , "NOTAS DE CREDITO", type: ToastType.Error, position: Position.TopCenter);
                }
            }
            ViewBag.ID_cliente = new SelectList(db.Cliente, "ID_cliente", "Razon_Social");
            ViewBag.CondicionVenta = new SelectList(db.Condicion_venta, "ID_condicion_venta", "Descripcion");
